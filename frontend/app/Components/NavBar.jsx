...
function NavBar() {
	...
	return (
		<div className={"customPagePad"}>
			<div
				className={`d-flex pt-2 mb-4 pb-2 align-items-center container-fluid NavBar`}
			>
				<div
					className={`d-flex align-items-center col-3 justify-content-evenly`}
				>
					<div>
						<button
							data-btn={isCollapsed}
							onClick={ToggleSidebarCollapseHandler}
							className="menu-btn w-auto h-auto bg-transparent"
						>
							...
							{sidebarItems.map(({ name, href, icon: icon }, index) => (
								<li key={index} className="sidebar__item">
									<Link href={href} className="sidebar__link">
										<span className="siderbar__icon">{icon}</span>
